    ...
    public override func layoutSubviews() {
        ...
        if dataSource != nil && delegate != nil {
            ...
            if let view = collectionController.collectionView, let layout = collectionController.collectionViewLayout as? HPCollectionViewFlowlayout {
                ...
                if let firstElement = delegate?.horizontalPickerView(pickerView: self, titleForRow: 0), let lastElement = delegate?.horizontalPickerView(pickerView: self, titleForRow: layout.lastElementIndex) {
                    layout.sectionInset.left  = layout.midX - collectionController.sizeForText(text: firstElement, maxSize: view.bounds.size).width / CGFloat(2)
