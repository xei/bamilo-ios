...
- (void)reportAppOpenToAdX:(bool)now {
    ...
    @autoreleasepool { // we're in a new thread here, so we need our own autorelease pool
        ...
        if(![fileManager fileExistsAtPath:appOpenPath]) {
            ...
            AdXLog(@"Ad-X: HTTP response %ld %@",(long)[(NSHTTPURLResponse *)response statusCode],respStr);
...
- (void)doReportAppOpen
{
    ...
    if((!ierror) && ([(NSHTTPURLResponse *)response statusCode] == 200) ) {
        ...
        NSNotification *not = [NSNotification notificationWithName:@"AdXDone" object:self];
...
- (BOOL)doSwish
{
    ...
    if ([self connectedToNetwork])
    {
        ...
        if (![[UIApplication sharedApplication] openURL:url]) {
            AdXLog(@"Ad-X: %@%@",@"Failed to open url:",[url description]);
        }
        return TRUE;
    }
    return TRUE;
...
- (BOOL)handleOpenURL:(NSURL *)url
{
    ...
    AdXLog(@"Ad-X: Got the URL as %@",URLString);
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	{
        AdXLog(@"Ad-X: Got Referral from server %@",referral);
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    if ([currentElement isEqualToString:@"Swish"])
	{
        ...
        if ([string compare:@"true"] == NSOrderedSame)
		{
			...
			{
                AdXLog(@"Ad-X: Swish is a go - %f seconds",interval);
                [self doSwish];
			}
            else
			{
                AdXLog(@"Ad-X: Connection took %f seconds - defer",interval);
                NSNotification *n = [NSNotification notificationWithName:@"AdXDone" object:self];
                [[NSNotificationCenter defaultCenter] postNotification:n];
			}
		}
        else
		{
            ...
            AdXLog(@"Ad-X: Ok - ready to send to Ad-X");
