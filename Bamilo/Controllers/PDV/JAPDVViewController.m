        ...
        [RIProduct getCompleteProductWithTargetString:self.targetString
                                    withRichParameter:richParameter
                                         successBlock:^(id product) {
                                             ...
                                             self.apiResponse = RIApiResponseSuccess;
                                             [self processLoadedProduct:product];
...
- (void)processLoadedProduct:(RIProduct*) product {
...
- (void)goToSellerLink {
    if(VALID_NOTEMPTY(self.product.seller, Seller)) {
        if ([self.product.seller isGlobal]) {
            NSMutableDictionary* userInfo = [[NSMutableDictionary alloc] init];
            [userInfo setObject:self.product.seller.name forKey:@"title"];
            [userInfo setObject:self.product.seller.target forKey:@"targetString"];
...
- (void)shareProduct {
    ...
    //
    UIActivityViewController *activityController = [[UIActivityViewController alloc] initWithActivityItems:@[STRING_SHARE_PRODUCT_MESSAGE, urlToBeShared] applicationActivities:nil];
