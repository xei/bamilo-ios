...
- (void)formSubmitButtonTapped {
    ...
    if(self.address.uid) {
        ...
        [[AddressDataManager sharedInstance] updateAddress:self params:params id:self.address.uid completion:^(id data, NSError *error) {
            if (error == nil) {
        ...
        [[AddressDataManager sharedInstance] addAddress:self params:params completion:^(id data, NSError *error) {
            if (error == nil) {
                [self.navigationController popViewControllerAnimated:YES];
            } else {
                if(![self showNotificationBar:error isSuccess:NO]) {
...
- (void)getAddressByID: (NSString *)uid {
    [[AddressDataManager sharedInstance] getAddress:self id:uid completion:^(id data, NSError *error) {
        if (error == nil) {
...
- (void)getRegionsByCompletion:(void (^)(void))completion {
    [[AddressDataManager sharedInstance] getRegions:self completion:^(id data, NSError *error) {
        if (!error)  {
...
- (void)getCitiesForRegionId:(NSString *)regionId completion:(void (^)(void))completion {
    [[AddressDataManager sharedInstance] getCities:self regionId:regionId completion:^(id data, NSError *error) {
        if (!error) {
...
- (void)getVicinitiesForCityId:(NSString *)cityId completion:(void (^)(void))completion {
    [[AddressDataManager sharedInstance] getVicinity:self cityId:cityId completion:^(id data, NSError *error) {
        if (!error) {
